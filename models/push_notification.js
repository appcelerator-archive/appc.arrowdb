...
module.exports = Arrow.Model.extend("appc.arrowdb/push_notification", {
	...
	methodMeta: {
		"notify": {
			"summary": "notify",
			"description": "Sends push notifications to one or more users who are subscribed to a channel.  Specify either `friends` or `to_ids`. Enterprise users can also specify `where`. One of these parameters must be used. These parameters cannot be used simultaneously.  Application admins can set the `to_ids` parameter to `everyone` to send to all devices subscribed to the identified channel.  The REST version of `notify` includes the push notification ID in its response. See the REST example below. ",
			"authRequired": true,
			"instance": true,
			"adminRequired": false,
			"response": {
		...
		"channelsQuery": {
			...
			"description": "Returns a list of push notification channels the user is subscribed to.  For application admins, if the `user_id` parameter is not specified, returns all channels with subscribed users. ",
			"authRequired": true,
			"instance": true,
			"adminRequired": false,
			"response": {
		...
		"subscribeToken": {
			"summary": "",
			"description": "Subscribes a mobile device to a push notifications channel.  Developers can create different channels for different types of push notifications, such as a friend request, chat, etc. ",
			"authRequired": false,
			"instance": true,
			"adminRequired": false,
			"response": {
		...
		"resetBadgeGet": {
			"summary": "reset_badge",
			"description": "Sets the internally stored value of the badge to zero for all devices.  **Only available to application admins.**  This method only updates the internally stored value of the badge.  To update the badge value on the iOS icon or Android notification center, send a push notification with the `badge` field defined.  To send this command to a specific device, use the [PUT version](#!/api/PushNotifications-method-reset_badge_put) of this method. ",
			"authRequired": false,
			"instance": true,
			"adminRequired": false,
			"response": {
