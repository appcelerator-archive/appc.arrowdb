...
module.exports = Arrow.Model.extend("appc.arrowdb/friend", {
	...
	methodMeta: {
		...
		"add": {
			"summary": "Add Friends",
			"description": "Add friends to the current user. By default the friend request is two-way (like Facebook), so after a friend request is made and approved both users will show up in each others' friend lists. This default can be changed to one- way following (like Twitter) in the App Settings for each of your apps.  Two-way or one-way friend requests must be approved by the recipient unless `approval_required=false` is also sent with the request. This allows the user to add any user as a friend without requiring approval. ",
			"authRequired": true,
			"instance": true,
			"adminRequired": false,
			"response": {
		...
		"search": {
			...
			"description": "Performs a search for users who are friends of the currently logged-in user. An application admin can search for friends of an arbitrary user by specifing the the `user_id` field.  Specifying the optional `q` parameter allows searching by first name, last name, email address, or username. If no `q` parameter is specified, all friends of the specified user are returned.  If one-way friendship is enabled, the search returns the users being followed. To return the user's followers, set the `followers` field to true.         ",
			"authRequired": true,
			"instance": true,
			"adminRequired": false,
			"parameters": [
