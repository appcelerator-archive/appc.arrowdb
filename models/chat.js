...
module.exports = Arrow.Model.extend("appc.arrowdb/chat", {
	...
	methodMeta: {
		...
		"queryChatGroups": {
			...
			"description": "Queries chat groups.  If user A sends a chat message to users B and C, users A, B and C automatically form a chat group. Use this API to get a list of chat groups the current user belongs to.  In ACS 1.1.5 and later, you can paginate query results using `skip` and `limit` parameters, or by including a `where` clause to limit the results to objects whose IDs fall within a specified range. For details, see [Query Pagination](#!/guide/search_query-section-query-pagination).          Prior to ACS 1.1.5 you can use `skip` and `limit` parameters, or `page` and `per_page` but  not both in the same query.          For details about using the query parameters, see the [Search and Query guide](#!/guide/search_query). ",
			"authRequired": true,
			"instance": true,
			"adminRequired": true,
			"response": {
