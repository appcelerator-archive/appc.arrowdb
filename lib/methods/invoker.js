...
exports.invoker = function invoker(method, isInstance, Model, obj, callback) {
	...
	if (isInstance) {
		if (!(obj instanceof Arrow.Instance)) {
			return callback && callback(new Error('Invalid instance'));
		}
		if (obj._deleted) {
			return callback && callback(new Error('Invalid has already been deleted'));
		}
	} else {
	...
	if (typeof Model[method] !== 'function' || (Model.methodMeta && Model.methodMeta[method] && Model.methodMeta[method].disabled)) {
		return callback && callback(new TypeError('unsupported method "' + method.replace(/^_/, '') + '"'));
	}
