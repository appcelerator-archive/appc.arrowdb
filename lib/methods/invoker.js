...
exports.invoker = function invoker (method, isInstance, Model, obj, callback) {
  ...
  var requestMethod = Model[method]
  if (typeof requestMethod !== 'function' || (Model.methodMeta && Model.methodMeta[method] && Model.methodMeta[method].disabled)) {
    return callback(new TypeError('unsupported method "' + method.replace(/^_/, '') + '"'))
  }

  // run the method
  requestMethod.call(Model, obj, callback, Model)
}
