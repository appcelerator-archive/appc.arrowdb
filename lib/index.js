...
exports.create = function create(APIBuilder, server) {
	...
	return APIBuilder.Connector.extend({
		...
		/**
		 * Wires up the built-in models and listens for an external models that
		 * need to be wired up.
		 */
		postCreate: function () {
	...
	function invoker(method, isInstance) {
		...
		return function (Model, obj, callback) {
			...
			if (method === '_query' && obj) {
				if (obj.sel) {
					obj.sel = translateToAllArray(obj.sel);
				}
				if (obj.unsel) {
					obj.unsel = translateToAllArray(obj.unsel);
				}
			}
