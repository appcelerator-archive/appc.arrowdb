	...
	return Arrow.Connector.extend({
		...
		loginRequired: function loginRequired(request, next) {
			if (!request.headers || !request.headers.sessioncookiestring) {
				...
				this.db.usersShowMe(function(err, user) {
					if (err) {
					...
					this.user = this.baseContext.user = user;
					next(null, false);
				}.bind(this));
			}
		},
		login: function login(request, response, next) {
			...
			this.db.usersLogin({
				...
				response.header('sessionCookieString', this.db.sessionCookieString);
				return next();
			}.bind(this));
		},
		...
		wireupModel: function (acsObj, model, name, isCustomObject) {
			...
			Object.keys(acsObj.methods).forEach(function (method, i, arr) {
				...
				model[destMethod] = function (instance, params, callback) {
					...
					this.getConnector().getDB()[methodInfo.apiMethodName].call(this.getConnector().getDB(), params, function (err, results) {
						if (err) {
		...
		fetchConfig: function fetchConfig(next) {
			if (!this.config.requireSessionLogin) {
				this.logger.warn('config.requireSessionLogin is turned off; requests can use the global ArrowDB authentication.');
			}
