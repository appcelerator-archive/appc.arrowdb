...
exports.create = function create(Arrow, server) {
	...
	return Arrow.Connector.extend({
		...
		models: Arrow.loadModelsForConnector('appc.acs', module),
		 ...
		 */
		postCreate: function () {
			var autogen = this.config.modelAutogen === undefined ? true : this.config.modelAutogen;
			if (server) {
			...
			Object.keys(acsObjects).forEach(function (name) {
				var acsObj = acsObjects[name],
					modelName = 'appc.acs/' + Arrow.singularize(acsObj.objectName || name),
		...
		connect: function connect(next) {
			this.logger.trace('logging into ACS as', this.config.username);
			this.acs.usersLogin({
				...
				this.logger.trace('session cookie:', this.acs.sessionCookieString);
		...
		fetchSchema: function (next) {
			if (!this.schema) {
				...
				Object.keys(acsObjects).forEach(function (name) {
					var singular = Arrow.singularize(acsObjects[name].objectName || name),
