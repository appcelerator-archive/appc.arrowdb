...
exports.create = function create(Arrow, server) {
	...
	var Collection = Arrow.Collection,
		...
		cacheMaxSize = (server && server.config && server.config.connectors['appc.arrowdb'] || {}).cacheMaxSize || 20,
		cache = new LRU(cacheMaxSize);
	...
	return Arrow.Connector.extend({
		...
		/**
		 * used for testing to reset the internal cache
		 */
		reset: function() {
			cache.reset();
			this.db = null;
		...
		loginRequired: function loginRequired(request, next) {
			...
			if (request.headers && request.headers.sessioncookiestring) {
				// check local cache
				var db = cache.get(request.headers.sessioncookiestring);
				if (db) {
					this.db = db;
					return next(null, false);
				}
				...
				cache.set(request.headers.sessioncookiestring, this.db);
		...
		login: function login(request, response, next) {
			...
			if (this.db && !username && !password && !request.cookies.arrowdbuid) {
				...
				return this.db.usersShowMe(function(err, result) {
					...
					this.user = result && result.body && result.body.response && result.body.response.users && result.body.response.users[0];
					if (this.user) {
						var setCookie = response.cookie;
						this.db.arrowdbuid = this.user.id;
						if (setCookie) {
							...
							if (!sent) {
								...
								setCookie('arrowdbuid', this.user.id);
							}
						}
					} else {
			...
			this.db.usersLogin({
				...
				if (err) {
					this.db = this.baseContext.db = null;
					return next(err);
				}

				this.user = this.baseContext.user = result.body.response.users[0];
				response.header('sessionCookieString', result.cookieString);
				this.db.sessionCookieString = this.baseContext.db.sessionCookieString = result.cookieString;
				return next();
			}.bind(this));
		},
		...
		connect: function connect(next) {
			...
			this.baseDB.usersLogin({
				...
				this.logger.trace('session cookie:', this.baseDB.sessionCookieString);
				this.baseUser = result.body.response.users[0];
				next();
			}.bind(this));
		},
		...
		findAll: function (Model, callback) {
			Model._query({ limit: 1000 }, callback, Model);
		},
		...
		count: function(Model, options, callback) {
	...
	function invoker(method, isInstance) {
		...
		return function (Model, obj, callback) {
			...
			if (method === '_query' && obj) {
				if (obj.sel) { obj.sel = translateToAllArray(obj.sel, 1); }
				...
				if (obj.limit) { checkLimit(obj.limit); }
			}
	 ...
	 * @param obj
	 */
	function translateToAllArray(obj, includeId) {
		if (_.isString(obj)) {
			var result = {};
