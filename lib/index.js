...
exports.create = function create(Arrow, server) {
	...
	return Arrow.Connector.extend({
		 ...
		 */
		postCreate: function () {
			var autogen = this.config.modelAutogen === undefined || this.config.modelAutogen,
				generateModelsFromSchema = this.config.generateModelsFromSchema === undefined || this.config.generateModelsFromSchema;
			if (server) {
				for (var modelName in this.models) {
					if (this.models.hasOwnProperty(modelName)) {
						var model = this.models[modelName];
						model.autogen = autogen === undefined ? model.autogen : autogen;
	 ...
	 */
	function checkWhereQuery(where, parent) {
		for (var key in where) {
			if (where.hasOwnProperty(key)) {
				if (key === '$regex') {
					if (where[key].substr(0, 3) === '^.*') {
						throw new Error('ArrowDB $like queries cannot begin with a wildcard; ' + parent + ' query is invalid.');
					}
				}
				else if (_.isObject(where[key])) {
					checkWhereQuery(where[key], key);
				}
			}
		}
	}
