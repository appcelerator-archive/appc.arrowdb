			...
			Object.keys(acsObj.methods).forEach(function (method, i, arr) {
				...
				model[destMethod] = function (instance, params, callback) {
					...
					if (/_update|_create/.test(destMethod) && params) {
						...
						try {
							var testFields = params;
							// if a custom object we need to validate the fields
							if (params.classname && params.fields) {
								testFields = params.fields;
							}
							// run the validators
							this.instance(testFields,false);
						}
						catch (E) {
							// validation failed
							return callback(E);
