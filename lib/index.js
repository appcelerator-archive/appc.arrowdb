...
function Connector(obj) {
	return Mobware.createConnector(obj||{},{
		...
		update: function(model, next){
			// TODO: doesn't seem like you can update a user other
			// than the one that is logged in. need to investigate
			var error = new Error("not supported");
			error.code = 404;
			return next(err);
		},
		delete: function(model, id, next) {
			// TODO: doesn't seem like you can delete a user other
			// than the one that is logged in. need to investigate
			var error = new Error("not supported");
			error.code = 404;
			return next(err);
...
function getACSObject(name, method) {
	var object = ACS[name],
		fn = object && object[method];
	if (!fn) {
		throw new Error('invalid ACS Object named: '+name);
	}
	return fn;
}
