...
exports.wireModel = function (acsObj, model, name, isCustomObject) {
  ...
  function createInstance (model, values, params) {
    ...
    model._instanceMethods && Object.keys(model._instanceMethods).forEach(function (method) {
      var destMethod = instance[method] ? '_' + method : method
      instance[destMethod] = model._instanceMethods[method].bind(instance)
    })
  ...
  Object.keys(acsObj.methods).forEach(function (method, i, arr) {
    ...
    model[destMethod] = function modelMethodWrapper (instance, params, callback) {
      ...
      if (/_update|_create/.test(destMethod) && params) {
        ...
        var customFields = params.custom_fields || {}
        if (typeof (customFields) === 'string') {
          customFields = customFields.split(',').map(function (k) { return k.trim() })
        }
      ...
      if (/createMany/.test(destMethod) && params) {
        var batchCreatePayload = []
        Object.keys(params).forEach(function (parameterName) {
          if (parameterName !== 'classname') {
            batchCreatePayload.push(params[parameterName])
            delete params[parameterName]
          }
        })
        params.json_array = batchCreatePayload
      }
      ...
      var dbFunc = db[methodInfo.apiMethodName]

      dbFunc.call(db, params, function (err, results) {
        if (err) {
          if (err.errorCode === 1004) {
            if (destMethod === 'count') {
              // Returns 0 if the model doesn't exist, i.e. if we try to get the count
              // for a newly created model, which there isn't a table created yet,
              // the errorCode will be 1004, but we don't want to return 404 as response,
              // instead of this we will return a zero count
              return callback(null, 0)
            }
            if (/^Error: Invalid object id/.test(err.reason)) {
        ...
        if (!response) {
          ...
          } else if (/createMany/.test(destMethod) && results.body.meta && results.body.meta.message) {
            var parsedMessage = {}
            var metaMessage = results.body.meta.message
            var messageRegex = /"(\w+)"\s*=>\s*(\d+)/gi
            var matches = messageRegex.exec(metaMessage)
            while (matches) {
              var propertyName = matches[1]
              var propertyValue = parseInt(matches[2])
              parsedMessage[propertyName] = propertyValue
              matches = messageRegex.exec(metaMessage)
            }
            return callback(null, {
              received: parsedMessage.n,
              inserted: parsedMessage.nInserted
            })
          }
        ...
        if (Array.isArray(response) && (!methodInfo.response || !methodInfo.response.singleElement)) {
          try {
            responseInstance = new (Arrow.Collection)(responseModel, response.map(function (item) {
              return createInstance(responseModel, item, params)
            }, this))
          } catch (E) {
            return callback(E)
          }
          return callback(null, responseInstance, results)
        }
        // single model
        ...
        try {
          responseInstance = createInstance(responseModel, response, params)
        } catch (E) {
          return callback(E)
        }
        return callback(null, responseInstance)
      }.bind(this))
    }
  }, this)
}
